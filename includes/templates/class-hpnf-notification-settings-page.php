 ...
 */
class Hpnf_Notification_Settings_Page extends User_Account_Page {
	...
	public function __construct( $args = [] ) {
		...
		//
		// Order 5, above the form at 10: the per-type options push this below the fold, and the
		// one control that needs a decision per device belongs where it is seen without scrolling.
		if ( hivepress()->hpnf_notification_push && get_option( 'hp_notification_push', true ) && ! hivepress()->hpnf_notification_push->get_keys() ) {
		...
		} elseif ( hivepress()->hpnf_notification_push && hivepress()->hpnf_notification_push->is_enabled() ) {
			$blocks['notification_push_setup'] = [
				'type'    => 'content',
				'_order'  => 5,

				'content' => '<div class="hp-notification-push" data-component="notification-push-setup" hidden data-label-enable="' . esc_attr__( 'Enable push notifications', 'notifications-for-hivepress' ) . '" data-label-enabled="' . esc_attr__( 'Push notifications are on for this device.', 'notifications-for-hivepress' ) . '" data-label-blocked="' . esc_attr__( 'Push notifications are blocked for this site in your browser settings.', 'notifications-for-hivepress' ) . '" data-label-working="' . esc_attr__( 'Waiting for your browser…', 'notifications-for-hivepress' ) . '"><button type="button" class="hp-button button button--primary"></button><span class="hp-notification-push__status"></span></div>',
			];
		}
